...
public class PromotionHandlersTests
{
    ...
    public async Task ApplyPromotionCommandHandler_ShouldCalculateDiscount_WhenValid()
    {
        ...
        // Use reflection or dynamic to check anonymous type properties in unit tests
        var type = result.GetType();
        var discountAmount = (decimal)type.GetProperty("DiscountAmount")!.GetValue(result)!;
        var finalAmount = (decimal)type.GetProperty("FinalAmount")!.GetValue(result)!;
        
        discountAmount.Should().Be(100m);
        finalAmount.Should().Be(900m);
    }
