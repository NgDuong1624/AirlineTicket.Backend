...
| **GET** | `/me` | Authenticated | Get the profile of the currently logged-in user including aggregate booking statistics (`bookingStats`). | None | `User { id: string, email: string, emailConfirmed: boolean, fullName: string, phone?: string, avatarUrl?: string, languagePreference?: string, lastLoginAt?: string, role?: string, roleId: number, status?: string, isActive?: boolean, airlineId?: string, airlineName?: string, createdAt: string, updatedAt: string, bookingStats?: UserBookingStats { totalBookings: number, totalSpent: number, lastMonthSpent: number, lastYearSpent: number } }` |
